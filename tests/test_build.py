...
def test_full_build(tmpdir):
    mktree(
        tmpdir,
        {
            ...
            'theme': {'sass/main.scss': 'body {width: 10px + 10px;}', 'assets/foobar.png': '*'},
        },
    )
    build(tmpdir, mode=Mode.production)
    assert gettree(tmpdir.join('dist')) == {
        'foobar': {'index.html': '/foobar.3389dae.png\n/theme/main.a1ac3a7.css\n/another/\n'},
...
def test_build_no_templates(tmpdir):
    mktree(tmpdir, {'pages': {'foobar.md': '### Whatever'}})
    build(tmpdir, mode=Mode.production)
    assert gettree(tmpdir.join('dist')) == {
        'foobar': {'index.html': '<h3 id="3-whatever">Whatever</h3>\n'},
    }
...
def test_simple_render(tmpdir):
    ...
    mktree(
        tmpdir,
        {
            'pages': {'foobar.md': foo_page, 'spam.html': '# SPAM', 'favicon.ico': '*'},
    ...
    som = {
        'pages': {
            ...
            'favicon.ico': {'uri': '/favicon.ico', 'infile': config.pages_dir / 'favicon.ico'},
        }
    }
    expected_tree = {
...
def test_build_default_placeholders(tmpdir):
    mktree(tmpdir, {'pages': {'posts/2032-06-01-testing.html': '# testing'}})
    config = Config(
        source_dir=str(tmpdir),
        tmp_dir=str(tmpdir.join('tmp')),
        foo='bar',
        defaults={
            '/posts/*': {
                ...
                'dict_attrs': {'d_attr': '{{ test_attr }}', 'd_foo': 'foo'},
                'list_attrs': ['foo', 'bar', '{{ title }}'],
                'list_of_dicts_attr': [{'a_key': '{{ title }}-{{ test_attr }}', 'b_key': 'foo bar'}],
            }
        },
    )
    ...
    assert {
        '/posts/2032-06-01-testing.html': {
            ...
            'test_attr': 'Brain J',
            'dict_attrs': {'d_attr': 'Brain J', 'd_foo': 'foo'},
            'list_attrs': ['foo', 'bar', 'Testing'],
            'list_of_dicts_attr': [{'a_key': 'Testing-Brain J', 'b_key': 'foo bar'}],
            'template': None,
            'content': '# testing',
            'pass_through': False,
        },
...
def test_placeholders_error(tmpdir):
    ...
    config = Config(
        ...
        tmp_dir=str(tmpdir.join('tmp')),
        foo='bar',
        defaults={'/posts/*': {'testing': '{{ title }}-{{ foobar }}'}},
    )
...
def test_build_simple_som(tmpdir):
    ...
    config = Config(
        ...
        tmp_dir=str(tmpdir.join('tmp')),
        foo='bar',
        defaults={'/posts/*': {'uri': '/foobar/{slug}.html'}},
    )
...
def test_render_error(tmpdir, caplog):
    mktree(tmpdir, {'pages': {'foobar.html': '{{ 1/0 }}'}, 'theme': {'templates/main.jinja': '{{ content }}'}})
    with pytest.raises(HarrierProblem):
        build(tmpdir, mode=Mode.production)
...
def test_yaml_render(tmpdir):
    mktree(
        tmpdir,
        {
            'pages': {'test.yml': 'template: foobar.jinja\nfoo:\n  bar: 42\n'},
            'theme': {'templates/foobar.jinja': 'foo bar: {{ page.foo.bar }}'},
        },
    )
    build(tmpdir, mode=Mode.production)
    assert gettree(tmpdir.join('dist')) == {'test': {'index.html': 'foo bar: 42\n'}}
